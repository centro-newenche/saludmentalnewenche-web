...
// Falls back to Cloudflare's public testing key in case the env var isn't
// set (e.g. local dev without a .env.local). Set VITE_TURNSTILE_SITE_KEY
// once you've created your widget in the Cloudflare dashboard.
const TURNSTILE_SITE_KEY = import.meta.env.VITE_TURNSTILE_SITE_KEY || TESTING_SITE_KEY;

export default function ContactForm() {
  const turnstileRef = useRef(null);
  const [turnstileToken, setTurnstileToken] = useState(null);
  const [status, setStatus] = useState(null); // null | "missing-captcha" | "success"
  ...
  function handleSubmit(e) {
    ...
    // TODO: POST your form fields + turnstileToken to your backend, which
    // must verify the token against Cloudflare's Siteverify API using
    // your SECRET key before treating the submission as trusted.
    ...
    >
      <div className="grid gap-10 md:grid-cols-2 md:items-start">
        ...
        <div>
          ...
          <h2 className="font-display mb-4 text-2xl font-semibold md:text-3xl" style={{ color: "var(--pine)" }}>
            Contáctanos
          </h2>
